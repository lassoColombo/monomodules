...
use ../lib/reconcile.nu [norm-url classify act-move act-trash act-clone]
use ../providers
...
# Interactively reconcile drift between the configured remotes and what's on disk.
...
export def main [
  ...
  for repo in $flagged {
    ...
    if ($pick | str starts-with "move") {
      $plan = ($plan | append { action: "move", desc: $"[($repo.source)] ($repo.name) → ($repo.correct)", from: $repo.path, to: ($repo.target_dir | path join $repo.correct) })
    } else if $pick == "trash" {
      $plan = ($plan | append { action: "trash", desc: $"[($repo.source)] ($repo.name)", repo: $repo.path, source_dir: ($src_of | get $repo.source | get dir), rel: $repo.name })
    } else if $pick == "clone" {
      $plan = ($plan | append { action: "clone", desc: $"[($repo.source)] ($repo.name)", url: $repo.remote, to: ($repo.target_dir | path join $repo.correct) })
    }
  }

  if ($plan | is-empty) {
    print $"\n(ansi yellow)nothing selected — no changes made(ansi reset)"
    return
  }
