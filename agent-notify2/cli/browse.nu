...
# your keybinding — `agent-notify2 browse wiring` prints it — which is the same
...
export def wiring []: nothing -> string {
    ...
    ([ "Add this to the `normal` mode of ~/.config/zellij/config.kdl:"
       ""
       "  bind \"Alt a\" {"
       $"      Run \"nu\" \"-n\" \"-c\" \"use ($root); agent-notify2 browse\" {"
       "          name \"agents\""
       "          floating true"
       "          close_on_exit true"
       ...
       ""
       "A floating pane is not required — `agent-notify2 browse` runs perfectly"
