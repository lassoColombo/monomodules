...
#   agent-notify2 name "fix-the-parser"
...
@example "a script reporting itself" { agent-notify2 report --state working --client nightly }
@example "…and handing back control" { agent-notify2 report --state awaiting --message "3 files changed" }
export def report [
...
@example "name the session for the work it is doing" { agent-notify2 name "explain-agent-notify" }
export def name [
    value: string
    --id: string
]: nothing -> record {
