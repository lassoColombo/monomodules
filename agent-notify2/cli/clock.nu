# `agent-notify2 clock …` — the periodic look, and whether it is happening.
#
...
@example "start checking every 30s" { agent-notify2 clock install }
...
@example "is the periodic check alive?" { agent-notify2 clock status }
