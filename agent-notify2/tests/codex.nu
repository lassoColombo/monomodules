...
export def main [] {
    ...
    let d = [
        (check "SessionStart then a prompt leaves it working"
               (agent-notify2 store get "sess-cx" | get state) "working")
    ...
    let rec = agent-notify2 store get "sess-cx"
    let e = [
    ...
    let f = [
        (check "SessionEnd forgets the agent" (agent-notify2 store get "sess-cx") null)
    ...
    let c = [ (check "every record here came from codex"
                     (agent-notify2 store list | get client | uniq) []) ]
