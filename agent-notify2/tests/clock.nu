...
const TMP = ($nu.temp-dir | path join "agent-notify2-tests-clock")
...
export def main [] {
    ...
    let a = [
        (check "the tick is the same refresh a human can type"
               ($tick | last | str contains "agent-notify2 surfaces refresh") true)
    ...
    let c = [
        (check-err "an interval that would be a busy loop is refused"
                   "busy loop" {|| agent-notify2 clock install --interval 5 })
    ]
