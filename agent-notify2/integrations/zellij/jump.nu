# `agent-notify2 jump <who>` — go to an agent's pane.
#
...
@example "which agent does this prefix mean?" { agent-notify2 jump find 6923c0 }
export def find [
    ...
    if ($hits | is-empty) {
        error make --unspanned {msg: $"agent-notify: no agent called '($who)' \(try: agent-notify2 store list\)"}
    }
...
# suite may do, and `agent-notify2 jump argv <who>` answers "what would that
...
@example "what would that do?" { agent-notify2 jump argv build-the-thing }
export def argv [
    who: string     # an agent's id, a unique prefix of one, or its name
] {
    let rec = find $who
...
@example "go to an agent by name" { agent-notify2 jump build-the-thing }
@example "…or by the start of its id" { agent-notify2 jump 6923c0bc }
export def main [
    who: string     # an agent's id, a unique prefix of one, or its name
] {
    let cmd = argv $who
