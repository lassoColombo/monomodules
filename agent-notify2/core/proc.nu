...
# signal we considered is a proxy for that fact, and every proxy is wrong in some
# case: a zellij pane outlives the process it held, a transcript file outlives the
# session that wrote it, and a quiet record means "idle" as often as "dead".
#
...
# `AGENT_NOTIFY_ID` gives for identity.
#
# ONCE PER SESSION, at SessionStart. A pid does not change, so there is nothing to
# repeat, and the rest of the hook path is untouched.
#
...
# "yes", and keep a dead agent forever in the belief that a stranger is our agent.
# A pid AND the second it started cannot be confused for anything else.
...
# the command is whatever follows them — which survives a path containing spaces.
...
# The agent's process, as {pid, started} — or null when we cannot tell, which the
# callers must read as "no proof" and never as "dead".
...
# Which of these processes are still running, as a list of pids. Only a pid whose
# START TIME still matches counts — a recycled number is a different process.
#
# Returns NULL when `ps` could not answer at all, which is different from an empty
# list: empty means "none of them are running", null means "do not act on this".
# `ps` distinguishes them itself — it exits 1 with nothing on stderr when every
# pid is simply absent, and complains on stderr when the question was malformed.
