...
#
# STDIN JSON is what every shipped client uses, and what the survey found nearly
# everywhere: Claude Code, Codex hooks, Gemini CLI, Cursor and Goose all write one
# JSON object to a hook's stdin. The exceptions need no help from here — Codex's
# older `notify` appended JSON as an argv element (one `from json` in the client
# that wants it), and Aider passes nothing at all, so its client would have to
# invent an identity rather than read one.
