...
#
# The schema is OPEN (plan.md §4.1): a closed one would make the core depend on
# every integration's fields, so adding an integration would mean editing core —
# which is exactly what opt-in integrations must not require. The core therefore
# guarantees a small set of fields and owns their policy, and everything else
# lives in a namespace named after its owner, integration or client alike.
#
# One thing is deliberately closed: the STATE VOCABULARY. Surfaces render states
# — three counters, three glyphs, an urgency order — and a bar cannot draw a state
# it has never heard of. A shared vocabulary is what an adapter adapts *to*, so
# closing it is the mechanism that makes agent-agnosticism work rather than a
# limit on it. Every agent maps its own concepts onto these four.
...
# Set by the store, never by a caller, and EXCLUDED from the `changed` comparison
# — otherwise every write would look like a change and `changed` would be useless
# as the gate the whole event path hangs off.
export const BOOKKEEPING = ["v" "updated_at" "state_since"]
...
# written in Rome and read in UTC would differ textually), and the store is JSON
# on purpose — a foreign agent must be able to read it without nushell. Forcing
# UTC also means lexicographic order is chronological order.
...
export def semantic [rec: record]: nothing -> record {
...
# Strict where it matters, open everywhere else. Validation runs on the RESULT of
# a write rather than on its input, which is what lets `patch` both create and
# update through one code path: either way, what ends up on disk must be valid.
export def validate [rec: record]: nothing -> record {
    ...
    # `zellij: {...}` from degenerating into a scatter of `zellij_tab_id` fields.
    let known = $CORE_REQUIRED ++ $CORE_OPTIONAL ++ $BOOKKEEPING
