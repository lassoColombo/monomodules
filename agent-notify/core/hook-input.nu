...
#
# STDIN JSON is what every integration-registry client uses, and what the survey
...
# json` in the client that wants it), and Aider passes nothing at all, so its
# client would have to invent a session id rather than read one.
...
# carries no current-session, which every client's `map` already treats as
# "ignore".
