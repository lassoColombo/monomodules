...
#
# This module is PURE STATE. It never touches zellij, SketchyBar, or any
# display, and it never dispatches: the entry point sequences
# write-then-repaint, so that a write from the command line repaints exactly
# like a write from a hook, and so that this file stays a cheap leaf anything
...
# early return, and the render-side model cache — with a single check in the one
# place able to answer it. When it comes back false the event ends, having cost
# the process floor and one read.
#
# NAMING — `read` rather than the obvious `get`: a def named after a builtin
# shadows that builtin for every module this one imports, whatever the order of
# the `use` statements, and the failure is a parse error somewhere else entirely
# (`get -o` inside session-schema.nu, which never mentions our name). Multi-word
...
# error: a display with one unreadable record should still paint the others.
...
def read-dir [dir: string]: nothing -> list<any> {
    ...
    # session-store whose last agent has just ended is exactly that case — the
    # directory outlives its contents. Found by the Codex suite, which drops its
    # only record and then reads the session-store; every display would have hit
    # it eventually.
...
# an agent dead is the store-garbage-collector's job, and doing it here would
...
# so a future "resume a recent session" has somewhere to look.
export def ended []: nothing -> list<any> { read-dir (ended-dir) }

# ── merging ───────────────────────────────────────────────────────────────────

def is-record [value: any]: nothing -> bool { ($value | describe) | str starts-with "record" }
...
# what keeps concurrent owners from clobbering each other. Anything else
# replaces.
#
# A null value DELETES its key. Without it `patch` would be incomplete: a field
# could be written but never removed, and callers would be driven to `set`,
# which replaces everything and reintroduces exactly the read-modify-write races
# that per-record files exist to avoid.
...
# `changed: false` means precisely "nothing happened", with no mtime moved and
# no file touched. If a heartbeat is ever wanted (an agent proving it is alive
...
def write [id: string, before: any, merged: record]: nothing -> record {
    let final = session-schema validate (session-schema stamp $before ($merged | merge {id: $id}))
    let changed = ($before == null) or ((session-schema comparable-fields $before) != (session-schema comparable-fields $final))
    if $changed { commit $id $final }
    { changed: $changed, before: $before, after: (if $changed { $final } else { $before }) }
}

# Merge `changes` into this agent's record, creating it when absent. The result
# is what gets validated, which is what lets one code path both create and
# update.
...
# one — `--resume` hands back the SAME id, and `--fork-session` exists to opt
# out of that. zellij does not destroy a session when you detach. tmux does not.
...
# hook, the pane by discovery, the pid by the walk — but the name is authored,
# once, by a human or by an agent following an instruction, and nothing ever
# says it again. Deleting the record deleted the only copy.

# How long an ended session waits before it is really gone. Long enough to cover
# coming back to something after a weekend, short enough that the directory
# never becomes an archive nobody asked for.
const KEEP_ENDED = 7day

# Expire on END, which is the only moment `ended/` can grow — so it costs once
# per session, never on the hot path, and the prune-daemon gains no new job. (On
# the 30s prune-daemon it would be 2,880 scans a day to delete something once a
# week.)
#
...
def expire-ended-sessions []: nothing -> nothing {
    let dir = ended-dir
    if not ($dir | path exists) { return }
...
# File this session away. What `SessionEnd` means, and what the
# store-garbage-collector does to an agent it can prove is gone — the same
# gesture either way, because a clean exit and an abrupt kill leave the same
# thing behind.
#
...
#
# WHAT RETURNS IS THE SESSION, NOT THE RUN IT WAS IN: the core fields (name,
...
# neither outlives it.
export def reopen-session [id: string]: nothing -> bool {
    let f = ended-file $id
    if not ($f | path exists) { return false }
    let rec = try { open --raw $f | from json } catch { null }
    rm --force $f
    if ($rec == null) or (not (is-record $rec)) { return false }
    commit $id (session-schema session-fields $rec)
    true
}
