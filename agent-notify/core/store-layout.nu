...
# a cosmetic one: `../../etc/passwd` must not escape the session-store
# directory, and two different ids must never land on the same file. v1's
# mapping does neither — it replaces `/` with `_`, so `a/b` and `a_b` collide.
#
# The rule here: an id that is already safe and short IS the filename, so the
# common case (a uuid) stays greppable; anything else keeps a stripped prefix
# for recognisability and takes a hash of the WHOLE id for uniqueness. The
# mapping is one-way on purpose — nothing needs to reverse it, because every
# record carries its own `id` field and that is the truth.
...
# that is not fussiness: an id of `.hidden` — or `../../etc/passwd`, which
# strips to `....etcpasswd` — would otherwise produce a dotfile, and a dotfile
# is invisible to the glob `list` walks. The record would be written, readable
# by id, and absent from every display. (Found by the step-1 suite, which is the
# whole reason it tries a traversing id.)
const SAFE_ID = '^[A-Za-z0-9_][A-Za-z0-9._-]{0,99}$'

export def encode-id [id: string]: nothing -> string {
    if ($id =~ $SAFE_ID) { return $id }
    # Strip rather than escape: the prefix is only a hint for a human browsing
    # the directory, and the hash — taken over the ORIGINAL id, not the stripped
    # form — is what actually keeps two ids apart.
    let hint = $id
