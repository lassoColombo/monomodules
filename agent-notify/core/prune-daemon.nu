...
# showing it.
#
# The prune-command is NOT a second pruning mechanism. It runs exactly the same
# `core/store-garbage-collector.nu` prune — the pid one — and then repaints. All
...
# verified). A hook lives ~30ms, so nothing it spawns can still be running a
# minute later.
#
...
#
# SO: launchd, which IS the periodic thing already running. `StartInterval`
# means there is no daemon to keep alive, no lock file, no pid to supervise, and
# no detaching trick — launchd starts a nu, it prunes and repaints, it exits. It
# survives logout and reboot, which a spawned process would not.
...
# The prune-command itself is just `displays refresh`: prune what is provably
# gone, then repaint whatever is left. Nothing here that a human could not type.
...
export def plist [interval: int]: nothing -> string {
    let args = prune-command | each {|a| $"        <string>($a)</string>" } | str join "\n"
...
export def install-job [interval: int]: nothing -> record {
    let f = plist-path
...
export def uninstall-job []: nothing -> record {
    let f = plist-path
    ^launchctl bootout $"(domain)/($LABEL)" | complete | ignore
...
# Loaded or not, and what it last did. `launchctl list` gives the pid (or `-`
# when it is between runs) and the last exit status, which is how a broken
# prune-command is noticed rather than silently doing nothing every 30 seconds.
export def status-of-job []: nothing -> record {
