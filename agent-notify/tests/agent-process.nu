...
#
# This suite can be honest about liveness because it is itself a process: it
# uses its OWN pid as the known-alive case, and a process it starts and lets
# finish as the known-dead one. Nothing here depends on which agent is running
# the tests, or on any agent running at all.
#
# The two rules being tested are in core/store-garbage-collector.nu, and the
# second matters more than the first: not knowing that an agent is dead must
...
const TMP = ($nu.temp-dir | path join "agent-notify-tests-proc")
...
export def main [] {
    ...
    let e = [
        ...
        (check "pruning again finds nothing to do" (store-garbage-collector sweep-dead-sessions) [])
    ]
