...
export def main [] {
    ...
    # ── the janitor ──────────────────────────────────────────────────────────
    agent-notify store patch "alive-1" {client: "claude", state: "working", proc: $me} | ignore
    agent-notify store patch "dead-1" {client: "claude", state: "working"
                                        proc: {pid: $gone, started: "whenever"}} | ignore
    agent-notify store patch "untracked-1" {client: "claude", state: "awaiting"} | ignore

    let dropped = janitor prune
    let left = agent-notify store list | get id | sort
    let e = [
    ...
    agent-notify store patch "cleared-old" {client: "claude", state: "awaiting", proc: $me} | ignore
    agent-notify store patch "cleared-new" {client: "claude", state: "working", proc: $me} | ignore
    let dropped2 = janitor prune
    let left2 = agent-notify store list | get id | sort
    let f = [
