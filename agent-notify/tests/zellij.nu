...
export def main [] {
    ...
    agent-notify store patch "me-1" {client: "claude", state: "working", name: "monomodules"} | ignore
    ...
    let one = agent-notify store get "me-1"
    ...
    let f = [
        (check "observe writes where we are into the store"
               (agent-notify store get "me-1" | get zellij.pane_id) "3")
        (check "…so an agent paints its own pane on its very first event"
               ($f1 | first | get wrote) 1)
    ]

    let f2 = dispatch project [(agent-notify store get "me-1")] [(agent-notify store get "me-1")] --table $spy
    let f_gate = [
        (check "nothing moved, nothing sent" ($f2 | first | get action) "skipped")
    ]

    let stored = agent-notify store get "me-1"
    ...
    agent-notify store patch "me-1" {zellij: null} | ignore
    agent-notify store patch "cli-1" {client: "other", state: "working", name: "from-the-cli"} | ignore
    let g = [
        (check "a CLI write goes through the seam: the surface ran and observed"
               (agent-notify store get "me-1" | get -o zellij | is-not-empty) true)
        ...
        (check "a CLI drop still answers whether there was anything to drop"
               (agent-notify store drop "cli-1") true)
    ]
