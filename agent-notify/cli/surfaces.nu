# `agent-notify surfaces` — what can show the store, and whether it is on.
#
...
@example "what can show the store?" { agent-notify surfaces }
...
@example "repaint after editing the config" { agent-notify surfaces refresh }
...
@example "create the bar items" { agent-notify surfaces install sketchybar }
...
@example "how do I keep the bar items?" { agent-notify surfaces wiring sketchybar }
