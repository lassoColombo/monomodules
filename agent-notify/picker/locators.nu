...
#
# THE SAME IDEA AS `core/dispatch.nu`'s `integration-registry`, one directory
# down and for the other half of an integration. A hand-written table of
# closures, because nushell has no first-class modules and a name cannot be
...
# else (D47), so it must not decide where the picker can TAKE you — switching
# the zellij display off means "stop renaming my panes", not "stop jumping to
# them".
#
...
export def integration-registry []: nothing -> record {
    { zellij: {info: $locate.INFO
               owns: {|rec| locate owns $rec }
               location-label:  {|rec| locate location-label $rec }
...
# fake locator answers the three questions out of a record, the same move
...
export def owner [rec: record, --table: record]: nothing -> any {
    ...
    for name in ($t | columns) {
        let l = $t | get $name
        let mine = try { do $l.owns $rec } catch { false }
        if $mine { return $l }
    }
    null
}
