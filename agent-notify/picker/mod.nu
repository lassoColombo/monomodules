...
# session-store, print, read a key — and everything between them is a pure
# function living in `rows.nu`, `layout.nu`, `preview.nu` or `keys.nu`, each
# with its own assertions. That is deliberate: an event loop is the one part a
# suite cannot event-loop, so it is kept to the three lines that must be here.
#
# It was FOUR until step 8. The preview used to dump the agent's real terminal,
# which is a subprocess per frame and the one part of a frame the suite could
# not assert; it is the stored message now (D58), so that call went and took the
# whole of `preview-of` out of this file with it.
#
#   loop {
#       records ← the store        re-read EVERY frame: 0.33ms, the list is live
#       rows    ← build, filter    pure
#       view    ← keep-in-view     pure: the selection still exists? scrolled?
#       preview ← the message      pure: markdown, cut to the pane at its offset
#       view    ← the offset back  pure: only the preview knows where it ends
...
# }` reads "there is no terminal" the same way it reads "nothing happened", so
# with stdin closed the loop turns over about 37,000 times a second forever —
# and back when a frame dumped a pane, each of those passes spawned a zellij
# client too. Measured. So the catch is TIMED instead: a timeout that did not
# take the timeout's worth of time was not a timeout.
#
# Timed rather than matched on the message text, because the wording belongs to
# nushell and a reworded error would otherwise mean the picker exits instantly
# on every heartbeat.
...
# Long enough that a quiet picker is nearly free — one 0.33ms session-store read
# per 2s, now that no frame spawns anything — and short enough that an agent
# changing state, or finally answering, shows up while you are still looking at
# it.
const REFRESH_EVERY = 2sec
...
const KEY_TIMEOUT = 200ms

# One turn of the loop, over and over. Returns the chosen ROW, or null.
def event-loop [query: string, table?: record]: nothing -> any {
    ...
    loop {
        ...
        let lay = layout measure (term size) ($shown | length)
        $view = rows keep-in-view $view $shown $lay.list
        ...
        # is built, the same way `keep-in-view` corrects the list's `top` above.
        let seen = preview of (rows selected $shown $view) $lay.preview $lay.width ($view.pv_top? | default 0)
        $view = ($view | merge {pv_top: $seen.at})
        let lines = layout render $shown $view $lay $seen.rows
        if $lines != $painted {
            print -n (tty paint $lines (layout caret $view))
            $painted = $lines
        }

        # See the header: the catch is timed, not trusted.
        let began = date now
        let ev = try { input listen --types [key] --timeout $REFRESH_EVERY } catch { null }
        if $ev == null {
            if ((date now) - $began) < $KEY_TIMEOUT {
                error make --unspanned {msg: ("browse needs a terminal — run it in one, "
                    + "or bind it to a floating pane with `agent-notify browse help-setup`")}
            }
            continue
        }
...
#
# No `-> any` return signature would be wrong here, but a `nothing -> any` one
# is fine: `event-loop` is what may throw, and it does so through the guard
# below. IT TAKES NO RECORDS. It reads the session-store itself, on every frame
# — because the list is LIVE, so a snapshot handed in at the door would be
# discarded before the first repaint and the parameter would be a lie.
export def choose [
    ...
    # reports the ORIGINAL error at the ORIGINAL span, so it looks exactly like
    # a `try` that does not work (§10).
    print -n (tty setup)
    let r = try {
        {ok: true, picked: (event-loop $query $table)}
