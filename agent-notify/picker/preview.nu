...
# elaboration.
export def of [row: any, height: int, width: int]: nothing -> list<string> {
    if ($row == null) or ($height < 1) or ($width < 1) { return [] }
    let said = $row.rec?.message? | default ""
    let body = markdown lay-out (markdown plain $said) $width $height
    if ($body | is-empty) { [$NOTHING] } else { $body }
}
