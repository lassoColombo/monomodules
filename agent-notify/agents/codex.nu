...
# and awaiting at once. One transport, one key.
#
...
# `tool_input.description` is documented as an optional human-readable reason
# for the approval; the tool name is the worst case, and still tells you what is
# held up.
def permission-message [hook_input: record]: nothing -> string {
    let tool = $hook_input.tool_name? | default "a tool"
    let input = $hook_input.tool_input? | default {}
...
export def to-operation [event: string, hook_input: record]: nothing -> record {
    ...
    match $event {
        ...
        # event on a `source` of startup, resume, clear or compact, so setting
        # it outright would knock a working agent back to idle on every
        # compaction.
        "SessionStart" => (patch $id {
            cwd: ($hook_input.cwd? | default "")
            codex: {transcript_path: ($hook_input.transcript_path? | default "")}
        } {state: "idle"})
        ...
        # the parent is still working. `PreToolUse` would only repeat what
        # `PostToolUse` says, and the compaction pair says nothing about who is
        # waiting for whom.
...
# ~9ms per event for nothing (plan.md §10). Takes no arguments at all, because
# the event arrives in the body along with everything else.
#
# THE HOOK MUST BE INVISIBLE. Codex reads exit codes — 2 denies the operation
# the hook fired for — so nothing here may fail and nothing may print.
export def main [] {
    try {
