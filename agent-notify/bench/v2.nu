...
#   transition  a real state change, which is what a display will eventually have
#               to act on
#   end         SessionEnd, which drops a record
#
# The session-store is a temp directory, so nothing here touches a live agent.
...
export def main [] {
    ...
    # Put the agent into `working` first, so the re-assert case measures a
    # genuine no-op rather than a create.
    with-env $vars { (hook-input) | ^$NU ...(hook "UserPromptSubmit" | skip 1) | complete | ignore }

    print ([
        (wall "PostToolUse — re-assert, changed:false" (hook "PostToolUse")
              --vars $vars --stdin (hook-input {tool_name: "Bash"}) --rounds 25)
        (wall "Stop — a real transition" (hook "Stop")
              --vars $vars --stdin (hook-input {last_assistant_message: "done"}) --rounds 25
              --prepare {|| with-env $vars { (hook-input) | ^$NU ...(hook "UserPromptSubmit" | skip 1) | complete | ignore } })
        (wall "SessionEnd — drop" (hook "SessionEnd")
              --vars $vars --stdin (hook-input {reason: "logout"}) --rounds 15
              --prepare {|| with-env $vars { (hook-input) | ^$NU ...(hook "SessionStart" | skip 1) | complete | ignore } })
        (wall "an event we do not handle" (hook "PreCompact")
              --vars $vars --stdin (hook-input) --rounds 15)
    ] | fmt | table --width 100)
}
